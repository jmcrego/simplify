...
class DecoderRNN_Attn(nn.Module):
    ...
    def forward(self, tgt_batch, enc_final, enc_outputs, teacher_forcing):
        ...
        attn_hidden = Variable(torch.zeros(1, self.B, self.H))

        for t in range(self.S-1):
            ...
            #print("dec_output_word[{}]={}".format(t,dec_output_word.shape))

        return dec_outputs, dec_output_words

    def forward_step(self, input_word, attn_hidden, dec_hidden, enc_outputs):
        ### input_word [batch_size] is the target word at instance t for all batches 
        #print("forward_step")
        #print("input_word={}".format(input_word.shape))
        #print("attn_hidden={}".format(attn_hidden.shape))
        #print("dec_hidden={}".format(dec_hidden[0].shape)) #[num_layers, batch_size, hidden_size], 
        #print("enc_outputs={}".format(enc_outputs.shape)) #[seq_size, batch_size, hidden_size]
        #print("dec_hidden[0]={}".format(dec_hidden[0].shape)) ### [num_layers, batch_size, hidden_size]     len(dec_hidden)==num_layers
        #print("dec_hidden[1]={}".format(dec_hidden[1].shape)) ### [num_layers, batch_size, hidden_size]     len(dec_hidden)==num_layers
        ...
        #concat input_emb; attn_hidden for input-feeding
        ...
        # Calculate attention from current last layer [-1] RNN h state [0] and all encoder outputs;
        # apply to encoder outputs to get weighted average
        last_h_state = dec_hidden[0][-1]
        attn_weights = self.attn(last_h_state, enc_outputs) ## attn weights for each encoder output
        attn_weights = attn_weights.unsqueeze(0) #resize to [1, batch_size, seq_length]
        #print("attn_weights={}".format(attn_weights.shape)) #[1, batch_size, seq_length]
        #print("enc_outputs={}".format(enc_outputs.shape)) #[seq_size, batch_size, hidden_size]
        context = attn_weights.transpose(1,0).bmm(enc_outputs.transpose(1, 0)) # batched multiplication [batch_size, 1, seq_length] x [batch_size, seq_length, hidden_size] = [batch_size, 1, hidden_size]
        context = context.squeeze(1)  #[batch_size, hidden_size]
        #print('context={}'.format(context.shape))
        # concatenate together rnn_output and context and apply layer (Luong eq. 5)
        cat_output_context = torch.cat((rnn_output, context), 1)
        #print('concat_output_context={}'.format(cat_output_context.shape))
        attn_hidden = torch.tanh(self.concat(cat_output_context)) #### this is what should be use for input-feed
        #print('attn_hidden={}'.format(attn_hidden.shape)) #[B, H]
        # Finally predict next token (Luong eq. 6)
        dec_output = self.output(attn_hidden)
        #print("dec_output={}".format(dec_output.shape))
        dec_output = F.log_softmax(dec_output, dim=1)
        #print("dec_output={}".format(dec_output.shape))
        attn_hidden = attn_hidden.unsqueeze(0) #[1, B, H]
    ...
    def cat_directions(self, h):
        ...
        # and h should be:
        #[layers x batch x (num_directions*dim)] 
        #dim is hidden_size/2
