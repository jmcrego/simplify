...
class Model(nn.Module):
    ...
    def __init__(self, cfg, voc, emb, dropout=0.0):
        ...
        ### model
        self.encoder = EncoderRNN(self.embeddings, self.cfg.cell, self.cfg.num_layers, self.cfg.bidirectional, self.cfg.hidden_units, dropout)
        if cfg.attention == 'none': 
            self.decoder = DecoderRNN(self.embeddings, self.cfg.cell, self.cfg.num_layers, self.cfg.bidirectional, self.cfg.hidden_units, dropout)
        else:
            self.decoder = DecoderRNN_Attn(self.embeddings, self.cfg.cell, self.cfg.num_layers, self.cfg.bidirectional, self.cfg.hidden_units, dropout, self.cfg.attention, voc.idx_ini, voc.idx_end, voc.idx_pad, voc.idx_unk)
